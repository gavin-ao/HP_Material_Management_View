  ...
  export default {
    ...
    onLoad(option) {
      ...
      wx.request({
        ...
        method: "POST",
        data: {productId: option.productId, preCtoId: option.preCtoId, sessionID: that.$store.state.board.sessionID},
    ...
    methods: {
      ...
      infoCommit() {
        var that = this;
        wx.request({
          url: that.$store.state.board.urlHttp + '/wechatapi/product/submitCto',
          method: "POST",
          data: {
            productId: that.productId,
            preCtoId: that.preCtoId,
            partsIds: that.partsIds.join(","),
            prices: that.totalPrice,
          ...
          success: function (res) {
            if (res.data.success) {
              wx.navigateTo({
                // url: '../infoCommit/main?productId='+this.productId +"&preCtoId="+this.preCtoId +"&partsIds="+this.partsIds.join(",")+"&prices="+this.totalPrice
                url: '../infoCommit/main?totalPrice=' + that.totalPrice
              })
            }
          }
        })
