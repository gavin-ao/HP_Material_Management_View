...
import data.driven.cto.business.wechat.WechatLoginLogService;
import data.driven.cto.business.wechat.WechatUserService;
...
import data.driven.cto.vo.wechat.WechatUserInfoVO;
...
import static data.driven.cto.common.Constant.USER_INFO_MAX_INTERVAL;
import static data.driven.cto.util.WXUtil.executeDecodeUserInfo;
import static data.driven.cto.util.WXUtil.getSessionKey;
...
public class WechatLoginApiController {
    ...
    private JSONObject syncUser(String encryptedData, String iv, String sessionID, WechatApiSessionBean wechatApiSessionBean) {
        ...
        if(userInfo == null){
            //如果用户信息为空，就新增用户到数据库中
            String wechatUserInfoId = wechatUserService.addUserInfo(userObj);
